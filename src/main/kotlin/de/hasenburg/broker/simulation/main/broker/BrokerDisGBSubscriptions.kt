...
class BrokerDisGBSubscriptions(
        brokerId: BrokerId,
    ...
    override fun processSubscriptionUpdate(message: CSubscriptionUpdate,
                                           resultChannel: SendChannel<Message>) {
        val now = startLog(message)
        val sub = message.subscription

        // has to store subscription even if not affected to calculate formerly affected for others
        val oldGeofence = updateSubscription(message.origin.id, sub)
        brokerDirectory.getBTargetsForOtherAffectedBrokers(brokerId, now, sub.geofence)
            .map { message.forward(it) }
            .forEach { resultChannel.add(it) }

        if (oldGeofence != null) {
            brokerDirectory.getBTargetsForOtherFormerlyAffectedBrokers(brokerId, now, oldGeofence, sub.geofence)
                .map { message.forward(it) }
                .forEach { resultChannel.add(it) }
        }
    ...
    override fun processSubscriptionRemoval(message: CSubscriptionRemoval,
        ...
        val geofence = removeSubscription(message.origin.id, message.topic)
        if (geofence != null) {
            brokerDirectory.getBTargetsForOtherAffectedBrokers(brokerId, now, geofence)
                .map { message.forward(it) }
                .forEach { resultChannel.add(it) }
        }
